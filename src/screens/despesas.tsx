...
export default function Despesas({ navigation }: DespesasScreenPorps) {
  ...
  const [contaBancaria, setContaBancaria] = useState<TBancoCadastro | null>(
    null
  );
  //Forma pagamento
  ...
  const criaNovaDespesa = async () => {
    ...
    const despesa: TDespesas = {
      ...
      valor_pago:
        parseFloat(formatarValor(valorBaixa)) -
          parseFloat(formatarValor(desconto)) +
          parseFloat(formatarValor(taxa)) +
          parseFloat(formatarValor(jurosMulta)) || 0, // Formato "00.00"
    ...
    try {
      for (const item of arrayProdutos) {
        const estoque: TEstoque = {
          ...
          obs_estoque:
            observacao + ` Estoque lançado com a despesa ${id_conta_rec}`,
          identificacao: `Ped_${id_conta_rec}`,
        };

        const estoqueResponse = await fetch(
          `/api/produtos/${item.id_produto}/estoque`,
          {
            method: "POST",
        ...
        if (!estoqueResponse.ok) {
          throw new Error(
            `Erro ao cadastrar o estoque do produto ${item.desc_produto}`
          );
        }
  ...
  const adicionarProduto = () => {
    if (produto && quantidadeProdutos) {
      const novoProduto: TProdutoPedido = {
        ...
        valor_total_produto: (
          parseInt(produto.valor_produto) * parseInt(quantidadeProdutos)
        ).toFixed(2),
        ...
        String(
          parseFloat(totalProdutos || "0") +
            parseFloat(quantidadeProdutos || "0") *
              parseFloat(produto.valor_produto || "0")
        )
      );
      ...
      {isLoading ? (
        ...
        <ScrollView style={styles.scrollView}>
          ...
          <Card mode="elevated" style={styles.cardPanel}>
            ...
            {checkedProduto ? (
              <View
                style={[styles.cardPanel, { padding: 0, overflow: "visible" }]}
              >
                <View
                  style={[
                    styles.cardPanelContent,
                ...
                {arrayProdutos.length ? (
                  <View
                    style={[
                      ...
                      { paddingVertical: 0, paddingLeft: 10, paddingRight: 0 },
                    ]}
                  >
                    <DataTable>
                      <DataTable.Header>
                      ...
                      {arrayProdutos.map((item, index) => (
                        <View
                          style={{
                            ...
                            flexDirection: "row",
                            alignItems: "center",
                            borderBottomColor: "rgba(20, 91, 145, 0.5)",
                            borderBottomWidth: 1,
                          }}
                        >
                          <DataTable.Row
                          ...
                          >
                            ...
                            <DataTable.Cell
                              style={{ justifyContent: "center", maxWidth: 80 }}
                              textStyle={{ fontSize: 11 }}
                            >
                              {item.unidade_produto}
                            </DataTable.Cell>
                            <DataTable.Cell
                              style={{ justifyContent: "center", maxWidth: 60 }}
                              textStyle={{ fontSize: 11 }}
                            >{`R$${Number(item.qtde_produto)}`}</DataTable.Cell>
              ...
              >
                ...
                >
                  ...
                  {dataContaBancaria.map((item) => {
                    return (
                      <Picker.Item
                        label={truncateText(item.nome_banco_cad, 17)}
            ...
            <View
              style={[styles.cardPanelContent, { marginBottom: 10, zIndex: 1 }]}
            >
              <Picker
                dropdownIconColor="#9E9E9E"
            ...
            >
              <View
                style={[
                  styles.cardInputs,
                  { flexShrink: 1, width: "100%", marginHorizontal: 0 },
                ]}
              >
                <Text
                  style={{
            ...
            <View
              style={[
                styles.cardPanelContent,
                { marginBottom: 10, alignItems: "center", zIndex: 1 },
              ]}
            >
              <Checkbox
                color="#145B91"
                  ...
                  <TextInput
                    ...
                    value={String(
                      parseFloat(formatarValor(valorBaixa)) -
                        parseFloat(formatarValor(desconto)) +
                        parseFloat(formatarValor(taxa)) +
                        parseFloat(formatarValor(jurosMulta)) || 0
                    )}
                    disabled
                  />
                </View>
        ...
        <Button
          ...
          disabled={
            !(
              ...
              contaBancaria &&
              nomeDespesa &&
              (checked ? checked && valorBaixa : valorBaixa) &&
              fornecedor
            )
          }
