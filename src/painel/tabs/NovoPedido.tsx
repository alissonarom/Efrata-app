...
const NovoPedido: React.FC<NovoPedidoScreenPorps> = () => {
  ...
  const [categoFinanceiras, setCategoFinanceiras] = useState<
    TCategoFinanceira[]
  >([]);
  ...
  useEffect(() => {
    if (pedidosContext) {
      ...
      pedidosContext.atualizarOrcamentos(
        cliente.id_cliente,
  ...
  const novoPedido = () => {
    return {
      ...
      status_pedido:
        type === "troca" || (type === "pedido" && isSwitchOn)
  ...
  const postProdutosPedido = async (
    idPedido: number,
  ...
  // Função para adicionar os produtos ao pedido
  const postProdutosOrcamento = async (
    idPedido: number,
    produtos: TProdutoPedido[]
  ) => {
    try {
  ...
  const adicionarProduto = () => {
    if (produto && quantidadeProdutos) {
      const novoProduto: TProdutoPedido = {
        ...
        valor_total_produto: (
          parseFloat(valorUnitario) * parseFloat(quantidadeProdutos)
        ).toFixed(2),
      ...
      setTotalProdutos(
        String(
          parseFloat(totalProdutos || "0") +
            parseFloat(quantidadeProdutos || "0") *
              parseFloat(valorUnitario || "0")
        )
      );
      setTotalDescontoProdutos(
        totalDescontoProdutos +
          (descontoProdutos ? parseFloat(descontoProdutos) : 0)
      );
  ...
  const criarContaReceber: any = async (idPedido: string) => {
    ...
    const despesa: TContaReceber = {
      ...
      valor_rec:
        type === "troca"
          ? "00.00"
          : (parseFloat(totalProdutos) - totalDescontoProdutos).toFixed(2),
      valor_pago: "00.00",
    ...
    try {
      ...
      const produtos = arrayProdutos.map((produto) => ({
        ...
        valor_unit_produto:
          type === "troca"
            ? "00.00"
            : parseFloat(produto.valor_unit_produto).toFixed(4),
        ipi_produto: produto.ipi_produto
          ? parseFloat(produto.ipi_produto).toFixed(2)
          : undefined,
        icms_produto: produto.icms_produto
          ? parseFloat(produto.icms_produto).toFixed(2)
          : undefined,
        valor_custo_produto:
          type === "troca"
            ? "00.00"
            : produto.valor_custo_produto
              ? parseFloat(produto.valor_custo_produto).toFixed(4)
              : undefined,
        peso_produto: produto.peso_produto
          ? parseFloat(produto.peso_produto).toFixed(2)
          : undefined,
        peso_liq_produto: produto.peso_liq_produto
          ? parseFloat(produto.peso_liq_produto).toFixed(2)
          : undefined,
      }));
      ...
      if (pedidosContext) {
        pedidosContext.atualizarPedidos(
          cliente.id_cliente,
          vendedor.id_vendedor
        );
      }
      ...
      console.error(
        "Erro ao criar o pedido ou cadastrar os produtos e parcelas:",
        error
      );
    } finally {
      setLoading(false);
    ...
    try {
      ...
      const produtos = arrayProdutos.map((produto) => ({
        ...
        desconto_produto: produto.desconto_produto
          ? parseFloat(produto.desconto_produto).toFixed(2)
          : undefined,
        ipi_produto: produto.ipi_produto
          ? parseFloat(produto.ipi_produto).toFixed(2)
          : undefined,
      ...
      console.error(
        "Erro ao criar o orçamento ou cadastrar os produtos e parcelas:",
        error
      );
    } finally {
      setLoading(false);
  ...
  const removerProduto = (index: number) => {
    setArrayProdutos((prevArray) => {
      ...
      const valorRemovido =
        parseFloat(produtoRemovido.valor_unit_produto) *
        parseInt(produtoRemovido.qtde_produto);
      ...
      const descontoRemovido = produtoRemovido.desconto_produto
        ? parseFloat(produtoRemovido.desconto_produto)
        : 0;
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View>
          ...
          <Text
            numberOfLines={1}
            ellipsizeMode="tail"
            style={{ color: "white", maxWidth: 130 }}
          >
      ...
      {isLoading ? (
        ...
        <ScrollView style={styles.scrollView}>
          ...
          <Card
            mode="elevated"
            style={[styles.cardPanel, { overflow: "visible" }]}
          >
            <View
              style={[
                styles.cardPanelContent,
            ...
            >
              ...
              <Text
                style={{
                  fontSize: 10,
                  alignSelf: "flex-start",
                  color: arrayProdutos.length ? "green" : "red",
                }}
              >
                obrigatório
              </Text>
            ...
            <View style={[styles.cardPanelContent, { overflow: "visible" }]}>
              <View style={{ flex: 1 }}>
                <View
                  style={[styles.cardPanelContent, { overflow: "visible" }]}
                >
                  <View
                    style={{
                      width: 100,
            ...
            {arrayProdutos.length ? (
              <View style={styles.viewCardPedido}>
                <DataTable>
                  <DataTable.Header>
                    <DataTable.Title style={{ paddingBottom: 0, flexGrow: 2 }}>
                      Produtos
                    </DataTable.Title>
                    <DataTable.Title
                      numeric
                  ...
                  {arrayProdutos.map((item, index) => (
                    ...
                    >
                      <DataTable.Row
                        key={index}
                        style={{ paddingHorizontal: 0, flexGrow: 1 }}
                      >
                        <DataTable.Cell
                          style={{ flexGrow: 2 }}
                        ...
                        <DataTable.Cell
                          ...
                          textStyle={{ fontSize: 11 }}
                        >
                          {Number(item.qtde_produto)}
                        </DataTable.Cell>
                        <DataTable.Cell
                          style={{ justifyContent: "center", flexGrow: 1 }}
                          textStyle={{ fontSize: 11 }}
                        >{`R$${Number(item.valor_unit_produto)}`}</DataTable.Cell>
                        <DataTable.Cell
                          style={{ justifyContent: "center", flexGrow: 1 }}
                          textStyle={{ fontSize: 11 }}
                        >{`R$${Number(item.desconto_produto)}`}</DataTable.Cell>
                        <DataTable.Cell
                          style={{ justifyContent: "center", flexGrow: 1 }}
                          textStyle={{ fontSize: 11 }}
                        >{`R$${(Number(item.valor_total_produto) - Number(item.desconto_produto)).toFixed(2)}`}</DataTable.Cell>
                      </DataTable.Row>
                      <IconButton
                        icon="delete"
                        iconColor="red"
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity onPress={() => handleSelectProduto(item)}>
                    <Text style={styles.dropdownItem}>{item.desc_produto}</Text>
                  </TouchableOpacity>
                )}
              />
            </View>
      ...
      <View style={styles.footer}>
        <View style={styles.subFooter}>
          ...
          <Text style={styles.textFooter}>
            R$ {totalProdutos ? totalProdutos : 0}
          </Text>
        </View>
        <View style={styles.subFooter}>
          <Text style={styles.textFooter}>Desconto</Text>
          <Text style={{ color: "#ff9090", fontWeight: "600" }}>
            -R$ {totalDescontoProdutos ? totalDescontoProdutos : 0}
          </Text>
        </View>
        ...
        <Button
          ...
          labelStyle={{
            fontSize: 15,
            fontWeight: "600",
            color: arrayProdutos.length ? "#145B91" : "darkgrey",
          }}
          buttonColor="white"
          textColor="#145B91"
          mode="contained"
